...
import { Plus, Trash2, Pencil, Search, ImagePlus, X, Loader2 } from "lucide-react";
...
export function ExtrasTab({ extras, extrasCategories, reload }: Props) {
  ...
  const saveExtra = async () => {
    ...
    const payload: any = {
      ...
      price_label: formPriceLabel.trim() || (priceBrutto === 0 ? "W cenie" : `${priceBrutto.toFixed(0)} zł`),
      requires_person_count: formRequiresPersonCount,
      duration: formCategory === "obsluga" ? formDuration || null : null,
      contents,
      food_cost: parseFloat(formFoodCost) || 0,
    };

    let error;
    if (editingId) {
      ({ error } = await supabase.from("extras").update(payload).eq("id", editingId));
    } else {
      ({ error } = await supabase.from("extras").insert(payload));
    }
  ...
  const getTypeLabel = (cat: string) => EXTRA_TYPES.find(t => t.value === cat)?.label || cat;
