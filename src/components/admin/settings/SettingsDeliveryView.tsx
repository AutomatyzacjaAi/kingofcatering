...
const SettingsDeliveryView = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      const { data, error } = await supabase
        ...
        .select("id, company_address_full, company_lat, company_lng, delivery_price_per_km, max_delivery_km, free_delivery_above_km")
        .limit(1)
        .single();
      if (data) {
        setSettingsId(data.id);
        setCompanyAddressFull((data as any).company_address_full || "");
        setCompanyLat((data as any).company_lat != null ? Number((data as any).company_lat) : null);
        setCompanyLng((data as any).company_lng != null ? Number((data as any).company_lng) : null);
        setPricePerKm(Number((data as any).delivery_price_per_km) || 3);
        setMaxDeliveryKm((data as any).max_delivery_km != null ? Number((data as any).max_delivery_km) : null);
        setFreeDeliveryAbove((data as any).free_delivery_above_km != null ? Number((data as any).free_delivery_above_km) : null);
      }
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      free_delivery_above_km: freeDeliveryAbove,
    };
